...
getTodos('../JSON/todos.json', (data, error) => { //dva parametra, jedno za kad je sve ok a drugi parametar za kada nam se desi greska
    ...
    if (error) { //if od stringa je true, ne moramo da upisujemo if err == true -> ako je err = true, ispisati tu gresku:
        console.log(error); //ispisuje ako je doslo do greske u todos.json
        // ako todos.json ne radi, nece ucitati ni fruits.json ni vegetables.json
    }
    else {
        console.log(data); //ispisuje podatke kojima moze dalje da raspolaze iz todos.json
        //nakon sto su se ispisali podaci iz todos.json, pozivamo sledeci fajl fruits.json
        getTodos('../JSON/fruits.json', (data, error) => {
            if (error) {
                console.log(error); //ispisuje ako je doslo do greske u fruits.json
                // ako fruits.json ne radi, nece ucitati ti vegetables.json
            }
            else {
                console.log(data); //ispisuje podatke kojima moze dalje da raspolaze iz fruits.json
                ...
                getTodos('../JSON/vegetables.json', (data, error) => {
                    if (error) {
                        console.log(error); //ispisuje ako je doslo do greske u vegetables.json
                    }
                    else {
                        console.log(data); //ispisuje podatke kojima moze dalje da raspolaze iz vegetables.json
                    }
                });
            }
        });
